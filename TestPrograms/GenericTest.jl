...
function simple_dominator_graph()
    ...
    nplus = ncos + x
    ntimes = ncos * nplus
    four_2_subgraph = nplus + ncos

    one_3_subgraph = (-sin(x)) + 1
...
function simple_factorable_subgraphs()
    ...
    temp = FD.extract_all!(FD.compute_factorable_subgraphs(graph))
    return graph, [
...
function compute_dominance_tables(graph::FD.DerivativeGraph{T}, compute_dominators::Bool) where {T<:Integer}
    ...
    doms = Dict{T,T}[]   #create one idom table for each FD.root
