...
struct PathIterator{T<:Integer,S<:FactorableSubgraph}
    ...
    PathIterator(subgraph::FactorableSubgraph{S}, edge::PathEdge{T}) where {T,S<:AbstractFactorableSubgraph} = new{T,FactorableSubgraph{S}}(subgraph, edge)
end
...
function validate_edge(a::FactorableSubgraph, edge::PathEdge)
    let edge_next::PathEdge{T}, count = 0
        for edge in nextEdges(a.subgraph, a.current_edge) #should always be parent edges because state.current_node != dominator(a.subgraph)
            if subset(dominance_mask(a.subgraph), reachable_roots(edge)) #don't have to check for overlap of variables here because first call to iterate verified that alll edges have variable overlap. Variable fields in edges may be reset by code using this iterator but only after the edge state has been considered by the iterator. A little ugly but the alternative of not having an iterator is worse.
                count += 1
                @assert count ≤ 1
                edge_next = edge
            end
        end
        @assert count == 1 #should always be a next edge (and only one valid edge), otherwise wouldn't have reached the else clause.
        return edge_next
    end
end
...
function Base.iterate(a::PathIterator{T,S}) where {T,S<:AbstractFactorableSubgraph}
    #verify that there is a path, i.e., that this path in the subgraph has not been destroyed by factorization
    current_edge = a.start_edge
    while top_vertex(current_edge) != dominator
        for edge in nextEdges(a.subgraph, current_edge)
            @assert length(edge) == 1 #should always be just one edge on the path
            if S === DominatorSubgraph
                #verify that the edge is part of the factorable subgraph, and that at least one of the reachable_variables of the subgraph has not been reset in the edge due to a previous factorization
                if !subset(dominance_mask(a.subgraph), reachable_roots(edge)) && overlap(reachable_variables(a.subgraph), reachable_variables(edge))
                    return nothing
                end
            else
                #verify that the edge is part of the factorable subgraph, and that at least one of the reachable_roots of the subgraph has not been reset in the edge due to a previous factorization
                if !subset(dominance_mask(a.subgraph), reachable_variables(edge)) && overlap(reachable_roots(a.subgraph), reachable_roots(edge))
                    return nothing
                end
            end
            current_edge = edge[1]
    ...
    #know there is a valid path so can return iterator tuple
    return (a, a.start_edge)
end

function Base.iterate(a::PathIterator{T,FactorableSubgraph{S}}, state::PathState{T}) where {T,S<:AbstractFactorableSubgraph}
    if state.current_node == dominator(a.subgraph)
        return nothing
    else
        let
            edge_next = validate_edge(a.subgraph,)
            return (edge_next, PathState(top_vertex(edge_next), edge_next))
        end
    end
end

Base.IteratorSize(::Type{PathState{T}}) where {T} = Base.SizeUnknown()
Base.IteratorEltype(::Type{PathState{T}}) where {T} = Base.HasEltype()
Base.eltype(::Type{PathState{T}}) where {T} = PathEdge{T}
