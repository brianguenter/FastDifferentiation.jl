...
struct PathEdge{T<:Integer,N,M}
    ...
    reachable_variables::BitVector
    reachable_roots::BitVector
    unique_id::UUID #This reliably distinguishes between two otherwise identical edges. Otherwise the only difference is that the BitVector arrays of two PathEdges use different physical arrays even if the values are identical. Relying on this to make Set operations work correctly is too subtle and fragile.
