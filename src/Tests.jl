...
"""
```
SymbolicUtils.istree(x::T) or SymbolicUtils.istree(x::Type{T})
...
exprhead(x)
If x is a term as defined by SymbolicUtils.istree(x), exprhead(x) must return a symbol, corresponding to the head of the Expr most similar to the term x. If x represents a function call, for example, the exprhead is :call. If x represents an indexing operation, such as arr[i], then exprhead is :ref. Note that exprhead is different from operation and both functions should be defined correctly in order to let other packages provide code generation and pattern matching features.

operation(x::T)
Returns the head (a function object) performed by an expression tree. Called only if SymbolicUtils.istree(::T) is true. Part of the API required for simplify to work. Other required methods are arguments and SymbolicUtils.istree

arguments(x::T)
Returns the arguments (a Vector) for an expression tree. Called only if SymbolicUtils.istree(x) is true. Part of the API required for simplify to work. Other required methods are operation and SymbolicUtils.istree
