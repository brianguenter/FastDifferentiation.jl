...
@testitem "derivative" begin
    ...
    @test derivative(x) == FD.Node(1)
    @test derivative(FD.Node(1)) == FD.Node(0)
end
...
@testitem "DerivativeGraph constructor" begin
    ...
    graph = FD.DerivativeGraph(roots)
    ...
    # for (FD.node, partials) in pairs(correct_partials)
    #     for (i, one_partial) in pairs(partials)
    #         f1 = to_symbolics(FD.partial_value(graph, FD.node, i))
...
@testitem "FD.edge_exists" begin
    ...
    @test !FD.edge_exists(graph, FD.PathEdge(1, 7, FD.Node(0), 2, 2)) #this edge is not in the graph
...
@testitem "add_edge! for FD.DerivativeGraph" begin
    ...
    new_edge = FD.PathEdge(1, 7, FD.Node(y), length(variables), length(roots))
...
@testitem "FD.compute_edge_paths" begin
...
@testitem "dom_subgraph && dom_subgraph" begin
    import FastDifferentiation as FD
    include("../TestPrograms/GenericTest.jl")
    ...
    computed = (
        FD.dom_subgraph(graph, 1, 1, idoms[1]),
        FD.dom_subgraph(graph, 1, 2, idoms[1]))

    @test computed[1] == r1_doms[1]
    @test computed[2] == r1_doms[2]

    computed = (
        FD.dom_subgraph(graph, 1, 3, pidoms[1]),
        FD.dom_subgraph(graph, 1, 4, pidoms[1]))
...
@testitem "FD.factor_order" begin
    using DataStructures
    import FastDifferentiation as FD
...
@testitem "FD.subgraph_edges with branching" begin
    import FastDifferentiation as FD
...
@testitem "FD.deconstruct_subgraph" begin
    import FastDifferentiation as FD
    include("../TestPrograms/GenericTest.jl")
...
@testitem "FD.make_factored_edge" begin
...
@testitem "FD.evaluate_subgraph" begin
    import FastDifferentiation as FD
    include("../TestPrograms/GenericTest.jl")
...
@testitem "FD.times_used FD.PathEdge" begin
    ...
    e = FD.PathEdge(1, 2, FD.Node(0), BitVector([1, 0, 1]), BitVector([0, 0, 1]))
    @test FD.times_used(e) == 2
    e = FD.PathEdge(1, 2, FD.Node(0), BitVector([1, 0, 0]), BitVector([0, 0, 1]))
    @test FD.times_used(e) == 1
    e = FD.PathEdge(1, 2, FD.Node(0), BitVector([1, 0, 1]), BitVector([1, 0, 1]))
...
@testitem "FD.path_sort_order" begin
    ...
    e1 = FD.PathEdge(1, 2, FD.Node(0), BitVector([1, 0, 1]), BitVector([0, 0, 1]))
    e2 = FD.PathEdge(3, 2, FD.Node(0), BitVector([1, 0, 0]), BitVector([0, 0, 1]))
    @test FD.path_sort_order(e1, e2) == true

    e3 = FD.PathEdge(3, 2, FD.Node(0), BitVector([1, 1, 0]), BitVector([0, 0, 1]))
...
@testitem "factor ℝ¹->ℝ¹ " begin
    ...
    tmp1 = make_function([FD.value(fedge)], FD.variables(graph))
    df(x) = tmp1(x)[1]

    graph = complex_dominator_graph()
    tmp2 = make_function([FD.root(graph, 1)], FD.variables(graph))
    origf(x) = tmp2(x)[1]
...
@testitem "sparse_jacobian" begin
    ...
    for index in CartesianIndices(dense)
        if sprse[index] != dense[index] #empty elements in sprse get FD.value FD.Node{Int64,0} whereas zero elements in dense get FD.value FD.Node{Float64,0}. These are not == so need special case.
...
@testitem "spherical harmonics jacobian evaluation test" begin
    ...
    mn_func = make_function(FD.roots(FD_graph), FD.variables(FD_graph))
    FD_func(vars...) = vec(mn_func(vars))

    graph_vars = FD.variables(FD_graph)
    sym_func = make_function(FD.jacobian(FD.roots(FD_graph), graph_vars), graph_vars)
...
@testitem "Chebyshev jacobian evaluation test" begin
    ...
    sym_func = make_function(FD.jacobian(FD.roots(FD_graph), FD.variables(FD_graph)), FD.variables(FD_graph), in_place=false)
...
@testitem "SArray return" begin
    ...
    mn_func1 = make_function(sph_jac, FD.variables(sph_func)) #return type of executable should be Array
    m, n = size(sph_jac)
    mn_func2 = make_function(SMatrix{m,n}(sph_jac), FD.variables(sph_func)) #return type of executable should be StaticArray
